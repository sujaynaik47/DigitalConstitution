...
function ConstitutionPage() {
    ...
    const fetchConstitutionData = async () => {
      try {
        ...
        const articlesArray = Object.entries(data).map(([artNo, details]) => ({
          // ArtNo stores the exact key from the JSON (e.g., "21A")
          ArtNo: artNo,
          Name: `Article ${artNo}: ${details.title}`, 
          ArtDesc: details.description,
          // Store the clean title for backend submission
  ...
  const handleSubmitOpinion = async (e) => {
    ...
    try {
      ...
      const postData = {
        // FIX: Ensure the articleNumber is the exact ArtNo/selectedArticleId (e.g., "21A")
        articleNumber: selectedArticleId, 
        // FIX: Use the clean OriginalTitle for the backend
...
//           Name: `Article ${artNo}: ${details.title}`, 
//           ArtDesc: details.description,
//           // FIX: Add OriginalTitle for clean backend submission
//           OriginalTitle: details.title 
//         }));
