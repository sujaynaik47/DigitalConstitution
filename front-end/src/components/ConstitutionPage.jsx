...
const ReplyPost = ({ post, level = 0, onAgree, onDisagree, onComment, onReply, highlightPostId }) => {
  const [showReplyForm, setShowReplyForm] = useState(false);
  const [replyText, setReplyText] = useState('');
  const [submitting, setSubmitting] = useState(false);
  const hasReplies = post.replies && post.replies.length > 0;
  const postRef = useRef(null);
  const isHighlighted = post.postId === highlightPostId;
  ...
  return (
    <div 
      ref={postRef}
      className={`${level > 0 ? 'ml-6 md:ml-8 mt-3' : 'mt-4'} relative ${
        isHighlighted ? 'ring-4 ring-blue-400 rounded-lg' : ''
      }`}
    >
      ...
      <div className={`${level > 0 ? 'pl-4 border-l-2 border-transparent hover:border-blue-200 transition-colors' : ''}`}>
        {isHighlighted && (
          <div className="bg-blue-100 text-blue-800 text-sm font-semibold px-3 py-2 rounded-t-lg">
            📍 Viewing this post
          </div>
        )}
        <PostCard
...
function ConstitutionPage() {
  ...
  // Handle navigation with article number, reference post, and specific post highlighting
  useEffect(() => {
    ...
    if (articleParam) {
      ...
      // Handle specific post highlighting
      if (postIdParam) {
        setHighlightPostId(postIdParam);
        setActiveTab('discuss'); // Auto-switch to discussion tab
      }
